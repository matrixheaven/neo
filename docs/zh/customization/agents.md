...
`AGENTS.md` 是 Neo 唯一的项目指令入口文件名，存储在目录中的文件名必须精确匹配。全小写或大小写混合的变体只是普通文档，不会形成指令作用域。任何地方都没有 `CLAUDE.md` 回退。项目指令是会话级状态：Neo 以持久化指令 epoch 的形式将其写入会话的 JSONL 事件流，绝不改写系统 prompt 或此前的请求字节。

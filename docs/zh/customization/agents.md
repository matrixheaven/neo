...
以下仍是普通文本：`@@./rules.md`；行内提及（如 `See @docs/rules.md`）；围栏代码内的导入样式文本；URL；环境变量表达式。

解析规则：

- 相对路径从导入文件所在目录解析；`~` 使用平台 home 目录。
- 最终规范化路径必须位于主工作区或 `$NEO_HOME` 内，否则拒绝（`Blocked: untrusted import`）。
- 导入源必须是常规 UTF-8 `.md` 文件；目录、设备、套接字、URL 和其他特殊文件都会被拒绝。
- 规范化路径驱动循环检测与去重；同一来源被多次导入时，仅在首次出现处展开。
- 导入内容在原位置替换指令行，并包裹 `<included_instructions path="...">` 来源标记；源内容保持精确的 UTF-8 文本。
...
- `$NEO_HOME/AGENTS.md` 及 `$NEO_HOME` 下的导入是用户全局，始终受信。

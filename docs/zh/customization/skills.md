...
一个 Skill 包是一个以 `SKILL.md` 为规范入口的目录。`SKILL.md` 是 Neo 唯一会自动加载进上下文的文件；任何辅助文件都必须在技能指令中显式点名，通常使用 `${NEO_SKILL_DIR}` 从包根目录定位。
...
| `slashCommands` | array | 为斜杠命令集成预留并解析的元数据；当前不会绑定为别名 |

> `type: flow` 的技能永远不参与自动激活；`disableModelInvocation: true` 同样会把技能从模型自动调用列表中排除。

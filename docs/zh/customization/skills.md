...
技能是可复用的 Markdown 指令包，让 Neo 把"怎么做某类任务"沉淀成文件。技能由 `SKILL.md` 定义，运行时按三层优先级扫描，可由模型自动激活或用 `/skill:<name>` 手动触发。核心实现见 `crates/neo-agent-core/src/skills/`。
...
一个 Skill 包是一个以 `SKILL.md` 为规范入口的目录。`SKILL.md` 是 Neo 唯一会自动加载进上下文的文件；任何辅助文件都必须在技能指令中显式点名。包作者可使用加载技能信封中展示的根相对路径，或 `${NEO_SKILL_DIR}` 占位符（后者继续支持）。

规范包结构使用这些路径：

| 路径 | 用途 |
| --- | --- |
| `SKILL.md` | 必需的技能 manifest 和指令；唯一自动进入上下文的入口 |
...
Skill 包内的资源目录不会被扫描成嵌套技能。如果一个包需要子技能，请把子技能放在 `skills/` 下。
...
加载顺序实际为：从当前二进制把内置技能解压到 Neo 管理的 `.builtin/` 目录，再依次注入 extra、user 层；user 层最后写入 `HashMap`，因此**用户技能可覆盖同名内置技能**。目录支持嵌套，父目录有自己的 `SKILL.md` 时，`skills/` 下的子技能名会被前缀化为 `parent/child`。如需自定义内置技能，请复制到 `.builtin/` 之外。
...
| 技能 | 说明 |
| --- | --- |
| `mcp-config` | 配置 MCP server、处理 OAuth 登录、编辑 `[[mcp.servers]]` |
...
- **`type: prompt` / `type: inline`**：直接移除该字段；所有技能使用相同的提示注入路径。

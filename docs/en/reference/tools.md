...
| `Terminal` | Drive a real PTY session: start / write / read / resize / stop. Suited to long-running interactive processes. `start` / `write` / `read` share one optional `yield_time_ms` (defaults 250 / 250 / 3000 ms, range `0..=30000`) that waits for incremental **raw PTY** output after admission and operation readiness; expiry returns current output with `status: running` and never stops the command. Admission queue wait stays unbounded and keeps the tool call pending. `timeout_secs` is valid only for `mode=start`; omit it for no command deadline, otherwise use `300..=3600`. After a timeout, increase or double it and retry. If it is already `3600` or duration is uncertain, omit it. Echo, ANSI, CR, and cursor control are not filtered. For control input, `input` must decode to one control character: Ctrl+C is U+0003, Ctrl+D is U+0004, and Ctrl+Z is U+001A. Printable escape text such as backslash-u-0-0-0-3 is sent literally, and control input has no portable signal guarantee. |

...
Save (`neo workflow save`) validates first and defaults to **no-clobber**; `--force` is required to overwrite a different definition. Builtin scope is not writable.
...
### Slash: bare (dynamic capability)

```text
/workflow
```

Grants **one session-scoped capability** for the next reviewed dynamic `RunWorkflow` tool call. The model then supplies a dynamic definition (`name`, `description`, `phases`, `script`, `args`). Capability is single-use and binds exact source and arguments; it does not authorize child effects.

Exact slash parsing only: `/workflowish` and prose containing `/workflow` do not grant capability.
...
| `neo.swarm(input)` | Heterogeneous (or homogeneous-template) child batch; **per-item `output_schema` required** |
...
Registered tools are eligible by default except a centralized deny set for orchestration/control surfaces, including (among others): `RunWorkflow`, `Delegate`, `DelegateSwarm`, `TaskPause` / `TaskResume` / `TaskStop` / `TaskAnswer`, plan/goal tools, and multi-agent control tools. Dedicated workflow APIs own children, user input, and control. `TaskOutput` targeting the **current** run is rejected to avoid recursive lock/path re-entry. Shell admission stays pending with no implicit timeout.
...
**Do not request credentials, API keys, or other secrets.** Answers are persisted in the local journal and are inspectable after restart. The run enters durable `awaiting_user`, releases active VM/worker admission, and remains visible in `/tasks` and CLI. Answer via:
...
List and inspect with `neo workflow list --scope builtin` and `neo workflow show <name>`. Launch with `/workflow code-review {"scope":"..."}` or `neo workflow run code-review --args-json '...'`.
...
6. Prefer named `/workflow <name>` for reviewed launches; use bare `/workflow` only when you want one dynamic model-authored run.
7. Inspect with `TaskOutput` views/cursors; prune only after dry-run preview.

## Next steps

- [Built-in Tools](../reference/tools.md) — `RunWorkflow`, `TaskOutput`, pause/resume/stop

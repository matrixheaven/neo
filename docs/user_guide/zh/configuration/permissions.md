...
Neo 在执行工具调用前，会根据当前权限模式决定是否需要用户审批。权限模式由 `config.toml` 的 `permission_mode` 字段、CLI 参数（`--auto` / `--yolo`）以及 TUI 中的 `/ask`、`/auto`、`/yolo`、`/permissions` 命令控制。

## 权限模式一览

Neo 实际运行时有 `Ask`、`Auto`、`YOLO` 三种命名模式，外加由 `EnterPlanMode` 工具激活的**计划模式**（Plan）。三者由 `PermissionMode` 枚举定义，计划模式作为额外的硬保护叠加在其上。

| 模式 | 字符串值 | 行为 |
| --- | --- | --- |
| **Ask** | `"ask"` | 默认。读类工具（`Read`/`List`/`Grep`/`Glob` 等）和已知安全命令自动放行；写、shell、工具调用一律弹出审批对话框 |
...
> 优先级：CLI 参数 `--yolo` / `--auto` 覆盖配置文件；两者不能同时使用。运行中可用斜杠命令实时切换，正在进行的轮次也会立即生效。
...
需要审批时，对话框由运行时持有的**单一请求**构建。**选项是动态的**：列表只包含运行时对该调用能兑现的动作——标签只是展示文案，真正执行的是请求中携带的类型化动作。

### 普通工具 / Shell 审批

普通工具与 Shell 审批可提供的动作：
...
普通工具/Shell 审批**不提供修订反馈**。仅当运行时能推导出安全的可复用范围时，才会出现会话/前缀授权；两者都不可用时，对话框只显示单次批准与拒绝。
...
- **Shell**：`<workspace> + <cwd> + <argv>`。`git status` 和 `git log` 是两个不同的键；`git status && git push` 这类复合命令会作为整体记录，不会拆分成单独的 `git status`。
- **File write/edit**：`<workspace> + <path> + <operation>`。Write 和 Edit 是两套独立键。
- **Tool**：`<workspace> + <fully-qualified tool name>`（主要针对 MCP 工具）。

> 跨工作区隔离：所有键都带工作区根路径，会话复用时不会泄漏授权。
...
2. **Auto / 后台 AskUser**：Auto 模式拒绝 `AskUserQuestion`、批准其余（包括 Plan/Goal 退出）；后台 `AskUserQuestion` 永不弹窗；`EnterPlanMode` 在所有模式下自动放行。
...
8. **兜底**：弹出审批对话框，只包含运行时对该调用能兑现的动作（单次批准、可选的会话/前缀授权、拒绝——普通工具无修订反馈）。

> 实时性：`/ask`、`/auto`、`/yolo`、`/permissions` 切换的模式会立刻生效——不需要取消当前轮次，下一次工具调用就会按新模式评估。

## 下一步

- [配置文件总览](config-files.md) — `permission_mode` 字段位置
- [服务商配置](providers.md) — 模型与端点

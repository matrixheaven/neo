...
Neo 把所有持久化数据集中放在 `~/.neo/`（或 `$NEO_HOME`）下。会话、配置、技能、提示词、主题、审批规则都按约定布局存放，便于备份、迁移和清理。
...
| `wd_<slug>_<hash12>` | `wd_` + 工作区目录 basename 的 slug + `_` + 绝对路径 SHA-256 的前 12 个十六进制字符 | `wd_neo_a1b2c3d4e5f6` |
...
> slug 规则：basename 转小写，非 `[a-z0-9._-]` 的字符替换为 `-`，去掉首尾 `-`，截断到 40 字符；为空时回退为 `workspace`。哈希保证同名但路径不同的工作区拥有独立的会话桶。
...
| `agents/main/wire.jsonl` | 主 agent 的完整事件流（`neo.session.jsonl` 格式，schema v1），包含持久化的指令版本（epoch） |
| `agents/main/plans/` | 主 agent 的计划文件 |
| `agents/main/goals/` | 主 agent 的目标文件 |
| `agents/main/tasks/` | 主 agent 的后台任务产物 |
| `workflows/<run_id>/run.json` | 不可变的启动元数据：identity、固定的定义版本（definition revision/source）、args、phases、关联时的 parent/checkpoint、启动方与来源（launch actor/source）、资源上限快照 |
| `workflows/<run_id>/journal.jsonl` | 追加式的状态迁移、调用记录、schema 修复标记、用户请求/回答、产物提交、最终结果、实际用量、恢复记录 |
| `workflows/<run_id>/artifacts/` | 由日志引用的内容寻址不可变载荷 |
| `workflows/<run_id>/recovery-quarantine/` | 异常截断的日志尾部隔离（内容哈希命名）；不是运行状态 |
| `agents/<agent_id>/...` | 子 agent（如 Delegate 产生的）对应记录 |

Workflow **run** 位于会话目录下，不属于对话记录或后台任务投影。`run.json` 是不可变的启动元数据。当前状态、控制信息与服务商实际用量只来自追加式的 `journal.jsonl`。过大的结果可能以产物（artifact）引用。`TaskOutput` 通过游标（cursor）分页读取日志（journal）与产物，绝不同步加载完整日志。
...
优先级为 `builtin < user < trusted project`。历史会话仍可读取；没有持久化 `workflows/<run_id>/` 的卡片只是投影，不能恢复。自动保留策略会回收超过 30 天、终态、无引用、未固定（pin）的存储。见 [Workflows](../guides/workflows.md)。
...
| `~/.neo/SYSTEM.md` | 系统提示词 | 可选；替换 Neo 内置基础系统提示词 |
| `~/.neo/APPEND_SYSTEM.md` | 系统提示词追加 | 可选；追加在基础系统提示词后 |
| `~/.neo/AGENTS.md` | 用户全局指令 | 始终受信；作为会话指令版本（epoch）加载（不改写系统提示词），见 [项目指令](../customization/instructions.md) |
| `~/.neo/approval_rules.json` | 前缀审批规则 | 见 [权限模式](permissions.md#前缀级layer-2) |
| `~/.neo/trust.json` | 项目信任 | 记录每个工作区是否被用户信任（当存在 `AGENTS.md` 等输入时触发）；门控项目 `AGENTS.md` 指令加载 |
...
可用 `ls ~/.neo/sessions/` 查看所有工作区桶，根据 slug 找到对应项目。
...
> `trust.json` 也保存在 neo home 下。删除后，所有「已信任项目」的决策都会丢失，下次启动需要重新确认。

## 下一步

- [配置文件总览](config-files.md) — `sessions_dir` 等字段定义
- [权限模式](permissions.md) — `approval_rules.json` 的语义
- [服务商与模型配置](providers.md) — API key 与端点配置

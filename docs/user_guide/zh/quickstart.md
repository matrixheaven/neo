...
| `cargo` / `rustfmt` / `clippy` | 随 Rust 附带 | 随 Rust 标准安装提供 |
| API Key | 至少一个服务商（provider） | 例如 `OPENAI_API_KEY` |
...
`cargo install` 会编译 release 二进制并安装到 `~/.cargo/bin/`。请确认该目录已加入 `PATH`（使用 rustup 时默认已配置）。
...
在任意目录执行 `neo` 进入交互式 TUI：

```bash
neo
```

首次运行会在 `~/.neo/config.toml` 生成默认配置文件。如果还没有配置服务商，TUI 会提示你先完成设置。

## 配置 API Key

Neo 只读取一个配置文件：`~/.neo/config.toml`（设置了 `NEO_HOME` 环境变量后为 `$NEO_HOME/config.toml`）。API Key 有两种配置方式。
...
> 安全提示：方式二会把 Key 明文写入磁盘，仅在明确接受风险时使用。
...
在提示符里输入问题后按回车发送。`Enter` 提交，`Alt+Enter` 或 `Ctrl+J` 换行。
...
`neo run` 以无界面模式（headless）执行：把提示词（prompt）作为参数传入，将结果以事件流打印到 stdout，适合脚本化调用。通过 `--output` 可以切换输出格式：
...
`neo update` 会下载当前平台对应的构建产物，校验 GitHub SHA-256 摘要和 staged 二进制版本，在当前可执行文件旁创建 `.bak` 备份，然后原子替换运行中的二进制文件。如果替换失败，会自动从备份恢复。
...
只有一个备份槽位。每次成功更新会覆盖前一次备份。`--rollback` 会恢复并一次性消耗备份，不涉及网络请求。
...
`neo uninstall` 删除运行中的 Neo 二进制文件，然后删除其 `.bak` 备份（如果存在），并可选删除 Neo home 目录（`~/.neo` 或 `$NEO_HOME`）。数据目录仅在用户明确输入 `y`/`yes` 或传入 `-y`/`--yes` 后才会删除。
...
- Windows：删除运行中的 `.exe` 会因访问冲突报错。请先关闭 Neo，再从另一个进程手动删除。`.bak` 和 Neo home 不受影响。
...
- [常见问题](guides/faq.md) — 安装、配置与使用的常见问题
- [命令行参考](reference/cli.md) — 全部 CLI 子命令与参数

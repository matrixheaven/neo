...
| 快捷键 | 动作 | 说明 |
| --- | --- | --- |
| `Enter` | `InputSubmit` | 提交当前提示词。 |
| `Tab` | `InputTab` | 触发自动补全。 |
| `Ctrl+C` | `AppClear` / `InputCopy` | 编辑器有选区时复制；否则清空编辑器、中断轮次或拒绝审批。 |
| `Ctrl+D` | `AppExit` | 提示词为空时退出；500 ms 内再按一次确认退出。 |
| `Ctrl+Z` | `AppSuspend` | 将 Neo 挂起到 shell 后台（`fg` 恢复）。 |
| `Esc` | `SelectCancel` | 关闭浮层或取消选择。 |
...
> Plan / Goal / Model 选择器还有 `TogglePlanMode` / `ModelPickerOpen` 动作，但默认未绑定按键，可通过命令面板或斜杠命令触发。
...
| 快捷键 | 动作 | 说明 |
| --- | --- | --- |
| `Ctrl+S` | `PromptSteer` | 在下一个自然断点用当前编辑器文本引导运行中的轮次；没有轮次时排队为后续消息。需 `stty -ixon`。 |
| `Alt+Up` | `EditNextQueuedMessage` | 把下一条排队消息取回编辑器编辑。 |
| `Ctrl+C` | `Cancel` / `Interrupt` | 取消或中断当前轮次；若有审批待处理则全部拒绝。 |
...
| `Ctrl+Space` | `TranscriptSelectionStart` | 进入对话记录条目选择。 |
| `Ctrl+Shift+Space` | `TranscriptSelectionClear` | 清除对话记录选择。 |
| `Shift+Up` / `Shift+Down` | 向上 / 向下扩展选择 | |
| `Shift+PageUp` / `Shift+PageDown` | 向上 / 向下翻页扩展选择 | |
| `Ctrl+C`（有选区时） | `TranscriptCopySelection` | 复制对话记录选区。 |

## 审批对话框

审批对话框打开时，输入由 `handle_pending_approval_input` 处理：
...
所有动作都暴露了稳定 ID（见各表「动作」列对应的配置 ID，如 `tui.input.submit`、`app.exit`）。在 `~/.neo/config.*` 中按上面各表中的键路径绑定，就可以覆盖默认值；冲突由 `KeybindingsManager` 检测并报告。

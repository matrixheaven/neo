...
Neo 会把每一次对话都保存为一份**本地 JSONL 记录**：可恢复、可分叉、可压缩、可导出。本页介绍会话（session）的持久化模型与常用操作。
...
会话记录与服务商（provider）无关：所有事件都被归一化为 `AgentEvent`，JSONL 永不依赖具体的服务商协议。
...
neo -r                          # 打开会话选择器（仅 TTY）
neo resume <session-id>         # 恢复指定会话；非 TTY 时打印其对话记录（transcript）
```
...
在交互界面输入 `/resume`，或按打开会话选择器的快捷键。选择器可在「当前工作区」与「所有工作区」之间切换作用域，选中会话后回车加载。
...
若想分叉的不是当前会话，需要先打开会话选择器：在 TUI 中输入 `/resume` 或按 `Ctrl+R`，在弹出的列表里用方向键选中目标会话，再按 `Ctrl+N` 就可以分叉并切换到新会话。
...
`summary` 由 Neo 在运行过程中自动生成；借助命名与摘要，可以在会话选择器中快速识别历史会话。
...
设置 `NEO_HOME` 环境变量可以整体迁移这些数据的位置。详见 [数据位置参考](../configuration/data-locations.md)。

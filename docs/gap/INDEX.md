...
- MCP is documented as a runtime boundary. `neo-agent-core` now has the
  adapter/provider abstraction, stdio JSON-RPC process adapter,
  discovery-to-`ToolSpec` bridge, namespaced `ToolRegistry` registration, and
  async call delegation. `neo-agent print` and `neo-agent run` load enabled
  stdio MCP entries from project config; long-lived sessions, HTTP/SSE
  transports, and hosted MCP management remain gaps.
- Session storage is implemented as JSONL event persistence in
  `neo-agent-core`; local tree fork/rename metadata and HTML export are wired,
  while hosted share, branch summaries, and richer compaction remain
...
- Sessions docs now describe JSONL event persistence, local session metadata
  fork/rename, and current CLI/session reader behavior.

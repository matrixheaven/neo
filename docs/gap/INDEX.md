...
- MCP is documented as a runtime boundary. `neo-agent-core` now has the
  ...
  discovery-to-`ToolSpec` bridge, namespaced `ToolRegistry` registration,
  persistent initialized stdio session reuse, and async call delegation.
  `neo-agent print` and `neo-agent run` load enabled stdio MCP entries from
  project config; HTTP/SSE transports and hosted MCP management remain gaps.
- Session storage is implemented as JSONL event persistence in
  ...
  compaction, and HTML export are wired, while hosted share, branch summaries,
  and model-generated/richer compaction remain pi-inspired future work.

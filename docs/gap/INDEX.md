...
- MCP is documented as a runtime boundary. `neo-agent-core` now has the
  ...
  delegation. `neo-agent print` and `neo-agent run` load enabled stdio,
  HTTP, and SSE MCP entries from project config; MCP resources,
  subscriptions, and hosted MCP management remain gaps.

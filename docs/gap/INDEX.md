...
| --- | --- | --- | --- |
| [`neo-ai`](neo-ai.md) | Provider-neutral `ChatRequest`, `RequestOptions`, `ModelRegistry`, stream events, tool schemas, fake provider, environment key helpers. | `pi-ai` documents broad provider discovery, credential resolution, tool streaming, reasoning, images, and context handoff. | Keep Neo docs focused on implemented Rust contracts and mark network adapters/model catalogs as future work. |
...
| [`neo-agent`](neo-agent.md) | Clap command surface, project `.neo/config.toml`, environment overrides, config setters, sessions commands, models/MCP list commands. | pi coding-agent docs cover settings, providers, sessions, TUI, MCP/resources, and trust. | Describe actual project-local config/session commands and mark interactive/TUI/trust as unimplemented. |
...
| [`xtask`](xtask.md) | Stable docs/xtask gate plus opt-in workspace gate and Markdown local-link validation. | pi has npm check/supply-chain automation and generated docs metadata. | Preserve a small Rust gate, validate docs links, and add future checks only when Neo has stable generated artifacts. |

## Cross-Cutting Gaps

- Provider adapters are not yet documented as runnable because the Rust network
  adapters are not implemented in this slice.
...
- Session storage is implemented as JSONL event persistence in
  `neo-agent-core`; tree branching, naming, export/share, and compaction remain

...
use controller_factory::{
    current_unix_timestamp, dialog_result_may_close, parse_timestamp, same_work_dir,
    startup_notices,
};
#[cfg(test)]
...
async fn fork_session_transcript(
    ...
    let loaded = load_session_transcript(child_id.clone(), config).await?;
    Ok(ForkedSessionTranscript::new(child_id, loaded))
}

#[cfg(test)]
mod test_cases {
    #[path = "fixtures_sessions.rs"]
    mod fixtures_sessions;
    pub use fixtures_sessions::*;
    #[path = "fixtures_skills.rs"]
    mod fixtures_skills;
    pub use fixtures_skills::*;
    #[path = "fixtures_approvals.rs"]
    mod fixtures_approvals;
    pub use fixtures_approvals::*;
    #[path = "fixtures_config.rs"]
    mod fixtures_config;
    pub use fixtures_config::*;
    #[path = "fixtures_controllers.rs"]
    mod fixtures_controllers;
    pub use fixtures_controllers::*;
    #[path = "fixtures_workflow.rs"]
    mod fixtures_workflow;
    pub use fixtures_workflow::*;
    #[path = "fixtures_transcript.rs"]
    mod fixtures_transcript;
    pub use fixtures_transcript::*;
    #[path = "fixtures_shell.rs"]
    mod fixtures_shell;
    pub use fixtures_shell::*;

    // Re-exported for the original `#[path]` test modules, which
    // referenced these names through this container's former `use` block.
    pub use crossterm::event::{KeyModifiers, MouseButton};
    // Interactive submodules the original `#[path]` test modules reach
    // through `super::<module>`; kept in scope by this container.
    use super::{log_events, prompt_completion, turn};

//! Turn lifecycle against the real web service: session creation returns
//! before the turn completes, concurrent sessions never cross-cancel,
//! follow-up input queues on the running turn, dropping the web subscription
//! keeps the background turn alive, and idle sessions start fresh turns whose
//! submitted user message is persisted exactly once.

use serde_json::{Value, json};

use super::http;
use super::provider::{Step, openai_response_sse};
use super::session_env::{create_session, snapshot, start_env, wait_for_phase};
use super::ws;

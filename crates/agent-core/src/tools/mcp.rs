...
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct McpHostedConfig {
    pub server_id: String,
}

#[async_trait]
pub trait HostedMcpClient: Send + Sync {
    ...
    async fn list_resources(
        ...
        Err(McpError::protocol(
            "hosted MCP client does not support resources/list",
        ))
    }
    ...
    async fn read_resource(
        ...
        Err(McpError::protocol(
            "hosted MCP client does not support resources/read",
        ))
    }

    async fn subscribe_resource(&self, _server_id: &str, _uri: &str) -> Result<(), McpError> {
        Err(McpError::protocol(
            "hosted MCP client does not support resources/subscribe",
        ))
    }

    async fn unsubscribe_resource(&self, _server_id: &str, _uri: &str) -> Result<(), McpError> {
        Err(McpError::protocol(
            "hosted MCP client does not support resources/unsubscribe",
        ))
    }

    async fn next_resource_update(&self, _server_id: &str) -> Result<McpResourceUpdate, McpError> {
        Err(McpError::protocol(
            "hosted MCP client does not support resource update notifications",
        ))
    }
}

#[derive(Clone)]
pub struct McpHostedToolAdapter {
    config: McpHostedConfig,
    client: Arc<dyn HostedMcpClient>,
}

impl McpHostedToolAdapter {
    #[must_use]
    pub fn new(config: McpHostedConfig, client: Arc<dyn HostedMcpClient>) -> Self {
        Self { config, client }
    }
}

#[async_trait]
impl McpToolAdapter for McpHostedToolAdapter {
    async fn list_tools(&self) -> Result<Vec<McpToolDefinition>, McpError> {
        self.client.list_tools(&self.config.server_id).await
    }
    ...
    async fn call_tool(
        ...
        self.client
            .call_tool(&self.config.server_id, name, arguments)
            .await
    }

    async fn list_resources(&self) -> Result<Vec<McpResourceDefinition>, McpError> {
        self.client.list_resources(&self.config.server_id).await
    }

    async fn read_resource(&self, uri: &str) -> Result<McpResourceRead, McpError> {
        self.client.read_resource(&self.config.server_id, uri).await
    }

    async fn subscribe_resource(&self, uri: &str) -> Result<(), McpError> {
        self.client
            .subscribe_resource(&self.config.server_id, uri)
            .await
    }

    async fn unsubscribe_resource(&self, uri: &str) -> Result<(), McpError> {
        self.client
            .unsubscribe_resource(&self.config.server_id, uri)
            .await
    }

    async fn next_resource_update(&self) -> Result<McpResourceUpdate, McpError> {
        self.client
            .next_resource_update(&self.config.server_id)
            .await
    }
}

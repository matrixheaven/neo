...
/// Screen row and display column of the first occurrence of `needle` in the
/// rendered frame (stripped of ANSI). `str::find` returns byte offsets, so
/// the column counts characters (== display cells for the ASCII needles here).

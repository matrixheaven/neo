...
impl NeoTui {
    ...
    pub fn render_terminal_frame_at(
        ...
        chrome_render
            .row_kinds
            .extend(std::iter::repeat_n(ChromeRowKind::Other, hint_count));
        debug_assert_eq!(
            chrome_render.lines.len(),
            chrome_render.row_kinds.len(),
            "hints extend row kinds"
        );
        let chrome_height = chrome_render.lines.len();
        let mut lines = self
            .transcript
...
fn selection_hint_line(width: usize, theme: &TuiTheme) -> String {
    ...
    let text = "selected · right-click or ctrl+c copy · click or ctrl+space clear";

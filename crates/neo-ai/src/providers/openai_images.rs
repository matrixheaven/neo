...
impl ImageGenerationClient for OpenAiImagesClient {
    async fn generate_image(
        ...
        let response = self
            ...
            .send()
            .await
            .map_err(|err| AiError::Stream(format!("transport error: {err}")))?;
        let status = response.status();
        if !status.is_success() {
            return Err(AiError::Stream(format!("http status {}", status.as_u16())));
        }
        let response = response
            .json::<OpenAiImagesResponse>()
            .await
            .map_err(|err| AiError::Stream(format!("invalid image response: {err}")))?;
        let images = response
            .data
            .into_iter()
            .map(|image| {
                let data = match (image.b64_json, image.url) {
                    ...
                    _ => {
                        return Err(AiError::Stream(
                            "image response did not include b64_json or url".to_owned(),
                        ));
                    }
                };
...
fn headers(api_key: &str) -> Result<HeaderMap, AiError> {
    ...
    let authorization = HeaderValue::from_str(&format!("Bearer {api_key}"))
        .map_err(|err| AiError::Stream(format!("invalid authorization header: {err}")))?;

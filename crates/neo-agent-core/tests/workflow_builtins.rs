...
use neo_agent_core::workflow::{
    ...
    check_definition, load_fixture, parse_fixture, resolve_builtin_definition, run_builtin_fixture,
...
async fn required_child_failure_aborts_builtin_without_placeholder_result() {
    for (name, args) in [
        ...
        (
            "deep-research",
            json!({"question": "workflow failure handling"}),
        ),
        (
            "large-refactor",
            json!({"spec": "workflow failure handling"}),
        ),
    ] {
        ...
        assert!(
            report.final_result.is_none(),
            "{name} returned a placeholder"
        );
    }
}

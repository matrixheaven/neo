...
/// The dialog widget (owns a clone of the state for rendering).
pub struct QuestionDialogWidget<'a> {
    state: &'a QuestionStateMachine,
    theme: TuiTheme,
}

impl<'a> QuestionDialogWidget<'a> {
    #[must_use]
    pub const fn new(state: &'a QuestionStateMachine, theme: TuiTheme) -> Self {
        Self { state, theme }
    }

    /// Compute the desired height of the dialog for a given terminal area.
    #[must_use]
    pub fn desired_height(&self, area: Rect) -> u16 {
        ...
        if self.state.on_submit_tab() {
            // Submit tab: preview all answers
            for q in &self.state.questions {
                let header = q.header.as_deref().unwrap_or("Q");
                lines += crate::wrap_width(&format!("{header}: {}", q.answer()), inner_width)
                    .len()
                    .max(1);
            }
            lines += 1; // blank
            lines += 1; // submit/cancel
        } else {
            let q = &self.state.questions[self.state.active_tab];

            // Header
            if let Some(header) = &q.header {
                lines += crate::wrap_width(header, inner_width).len().max(1);
            }
            // Body
            if let Some(body) = &q.body {
                for l in crate::wrap_width(body, inner_width) {
                    lines += 1;
                    let _ = l;
                }
            }
            // Question
            lines += crate::wrap_width(&q.question, inner_width).len().max(1);
            lines += 1; // blank

            // Options (visible window)
            let total = q.total_options();
            let visible = total.min(MAX_OPTIONS_VISIBLE);
            lines += visible;
            if total > MAX_OPTIONS_VISIBLE {
                lines += 1; // pagination line
            }
            lines += 1; // blank
            lines += 1; // footer hint
        }
        ...
        // border (2) + content
        let total = lines.saturating_add(2).min(usize::from(area.height));
        u16::try_from(total).unwrap_or(area.height)
    }
}

impl Widget for QuestionDialogWidget<'_> {
    fn render(self, area: Rect, buf: &mut Buffer) {
        let theme = self.theme;
        let state = self.state;

        Clear.render(area, buf);
        buf.set_style(area, Style::default().bg(theme.approval_bg));

        let title = if state.on_submit_tab() {
            " Review & Submit "
        } else {
            " Questions "
        };
        ...
        let text_width = usize::from(inner.width.max(1));
        let mut y = inner.y;
        ...
        for (i, q) in state.questions.iter().enumerate() {
            ...
            if is_active {
                ...
                tab_spans.push(Span::styled(format!(" ({icon}) {label} "), style));
            }
        }
        // Submit tab
        ...
        tab_spans.push(Span::styled(" [Submit] ", submit_style));

        Paragraph::new(Line::from(tab_spans)).render(
            Rect {
                x: inner.x,
                y,
                width: inner.width,
        ...
        y = y.saturating_add(2); // tab bar + blank
        ...
        if state.on_submit_tab() {
            for q in &state.questions {
                ...
                let header = q.header.as_deref().unwrap_or("Q");
                let answer = q.answer();
                let answer_display = if answer.is_empty() {
                    "(unanswered)"
                } else {
                    &answer
                };
                let wrapped = crate::wrap_width(&format!("{header}: {answer_display}"), text_width);
                for line in &wrapped {
                    if y >= inner.bottom() {
                        break;
                    }
                    let style = if answer.is_empty() {
                        Style::default().fg(theme.warning)
                    } else {
                        Style::default().fg(theme.success)
                    };
                    Paragraph::new(Line::from(Span::styled(format!("  {line}"), style))).render(
                        Rect {
                            x: inner.x,
                            y,
                            width: inner.width,
            ...
            // Body
            if let Some(body) = &q.body {
                for line in crate::wrap_width(body, text_width) {
                    if y >= inner.bottom() {
                        break;
                    }
                    Paragraph::new(Line::from(Span::styled(
            ...
            // Question
            if y < inner.bottom() {
                for line in crate::wrap_width(&q.question, text_width) {
                    if y >= inner.bottom() {
                        break;
                    }
                    Paragraph::new(Line::from(Span::styled(
            ...
            let visible_end = (scroll + MAX_OPTIONS_VISIBLE).min(total);
            for i in scroll..visible_end {
                if y >= inner.bottom() {
                    break;
                }
                let is_other = i == q.options.len();
                let is_cursor = i == state.cursor;
                let is_selected = if is_other {
                    q.other_selected
                } else {
                    q.selected.get(i).copied().unwrap_or(false)
                };

                let label = if is_other {
                    if state.other_editing && is_cursor {
                        format!("Other: {}", state.other_input)
                    } else if q.other_selected && !q.other_text.is_empty() {
                        format!("Other: {}", q.other_text)
                    } else {
                        "Other".to_owned()
                    }
                } else {
                    q.options[i].label.clone()
                };

                let (prefix, style) = if q.multi_select {
                    ...
                    (format!(" {cursor_marker} [{check}] "), style)
                } else {
                    let num = i + 1;
                    let cursor_marker = if is_cursor { "→" } else { " " };
                    let base_style = if is_selected {
                    ...
                    (format!(" {cursor_marker} [{num}] "), style)
                };
            ...
            if total > MAX_OPTIONS_VISIBLE {
                if y < inner.bottom() {
                    Paragraph::new(Line::from(Span::styled(
                        format!("  showing {}-{} of {}", scroll + 1, visible_end, total),
        ...
        // --- Footer hint ---
        if y < inner.bottom() {
            let hint = if state.other_editing {
                "Type answer · Enter confirm · Esc cancel edit"
            } else if state.on_submit_tab() {
                if state.is_complete() {
                    "Enter submit · Esc cancel"
                } else {
                    "← → tabs · Esc cancel · answer all to submit"
                }
            } else {
                "↑↓ navigate · 1-9 select · Enter confirm · ← →/Tab tabs · Esc cancel"
            };
            Paragraph::new(Line::from(Span::styled(
                format!(" {hint}"),
                Style::default().fg(theme.notice),
...
// Re-export the `Clear` widget from ratatui for use in the render impl.
